...
RSpec.describe GroupsController, type: :routing do
  ...
  context "save_cover_photo route" do
    let(:group) { FactoryGirl.create :valid_group }

    it 'PUT group_controller#save_cover_photo' do
      expect(:put => groups_save_cover_photo_path(group)).to route_to(
        controller: 'groups',
        action: 'save_cover_photo',
        id: "#{group.id}"
      )
    end
  end
