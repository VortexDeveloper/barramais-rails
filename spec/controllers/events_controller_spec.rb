...
RSpec.describe EventsController, type: :controller do
  ...
  let(:event) { FactoryGirl.create :valid_event, user_id: user.id, address_id: address.id }
  ...
  describe "save_cover_photo" do
    it "updates the event with an cover_photo" do
      event.cover_photo = image
      timestamp = Time.now.strftime("%Y%m%d%H%M%S")
      put :save_cover_photo, params: {id: event.id, cover_photo: {image: image, filename: "#{event.id}#{timestamp}"}}
      event.reload

      expect(event.cover_photo).not_to be_nil
    end
  end
  ...
  let(:valid_attributes) {
    FactoryGirl.attributes_for :valid_event, user_id: user.id, address_id: address.id
  }
  ...
  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) { FactoryGirl.attributes_for :valid_event, user_id: user.id, address_id: address.id }
